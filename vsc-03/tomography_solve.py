...
def create_sinogram(nAngles, nSamples, angle_range=(0, np.pi)):
    ...
    ray_position = np.zeros((nSamples, 2))
    ray_direction = np.zeros((nAngles, nSamples))
    degrees = np.degrees(angle_range)
    for index,theta in enumerate(np.linspace(degrees[0], degrees[1], nAngles)):
        #print("X:", np.cos(i)*1, "-", "Y:", np.sin(i)*1)
    ...
    #       sinogram[i,j] =  tomograph.trace(rd,rp)
...
for i,ng in enumerate(gridsizes):
    ...
    nSamples = 2 * nGrid
    nAngles = 2 * nGrid
    create_sinogram(nAngles, nSamples, (0,np.pi*2))
